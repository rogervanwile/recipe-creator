    ...
    return <>
        {(
            props.attributes.preparationStepsGroups.map((group, index) => {
                return (
                    <div key={"preparationStepsGroups_" + index} className="foodblogkitchen-toolkit--recipe-block--editor foodblogkitchen-toolkit--recipe-block--preparation-steps">
                        {
                            (
                                (index !== 0 || props.attributes.preparationStepsGroups.length > 1) ?
                                    <div className="foodblogkitchen-toolkit--recipe-block--group-header">
                                        ...
                                        />
                                        <Button isTertiary={true} onClick={() => removeGroup(index)}>{__("Remove Group", "foodblogkitchen-toolkit")}</Button>
                                    </div> : ''
                            )
                        }
                        <RichText
