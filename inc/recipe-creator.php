...
class RecipeCreator
{
    ...
    function __construct()
    {
        ...
        add_image_size('recipe-creator--thumbnail', get_option('recipe_plugin_for_wp__thumbnail_size', $this->thumnailSizeDefault));
        ...
        add_action('wp_ajax_nopriv_recipe_plugin_for_wp_set_rating', array($this, 'setRating'));
...
?>
            <div class="notice notice-info is-dismissible">
                <p>
                    ...
                    echo sprintf(
                        ...
                        __('We\'ve updated the <strong>Recipe Creator</strong> plugin. Check out the <a href="%s">release notes</a> to see which new features you can use now.', 'recipe-creator'),
        ...
        if (is_singular() && in_the_loop() && is_main_query()) {
            if (has_blocks()) {
                ...
                if (has_block('recipe-creator/recipe')) {
                    ...
                    if (get_option('recipe_plugin_for_wp__show_jump_to_recipe', true) && !has_block('recipe-creator/jump-to-recipe')) {
    ...
    public function fetchInfo($res, $action, $args)
    {
        ...
        if (false == $remote = get_transient('recipe_plugin_for_wp_update')) {
            ...
            $remote = wp_remote_get('https://updates.howtofoodblog.com/recipe-creator/info.json', array(
                'timeout' => 10,
                'headers' => array(
    ...
    private function renderColorPickerInput($name, $defaultValue)
    {
        ...
        echo '<input type="text" class="recipe-creator--color-picker" name="' . $name . '" value="' . $value . '" data-default-value="' .  $defaultValue . '" />';
    }
    ...
    public function registerRecipeBlockSettings()
    {
        ...
        add_settings_section(
            ...
            function () {
                echo '<p>' . __("Configure how the recipe block should behave on your blog posts.", 'recipe-creator') . '</p>';
            },
            'recipe_plugin_for_wp__general'
        );
        add_settings_section(
            ...
            function () {
                echo '<p>' . __("Provide informations about your instagram profile and we show a call to action below your recipe.", 'recipe-creator') . '</p>';
            },
            'recipe_plugin_for_wp__general'
        );
        add_settings_section(
            ...
            function () {
                echo '<p>' . __("Configure how the recipe block should look for your visitors.", 'recipe-creator') . '</p>';
            },
            'recipe_plugin_for_wp__general'
        );
        ...
        add_settings_field(
            ...
            function () {
                $this->renderCheckboxInput('recipe_plugin_for_wp__show_jump_to_recipe', true, __('Add a "Jump to recipe" button on every page with the recipe block.', 'recipe-creator'));
            },
            'recipe_plugin_for_wp__general',
            'recipe_plugin_for_wp__general',
            array(
        ...
        add_settings_field(
            ...
            function () {
                $this->renderCheckboxInput('recipe_plugin_for_wp__show_border', $this->showBorderDefault, __('Show border', 'recipe-creator'));
            },
            'recipe_plugin_for_wp__general',
            'recipe_plugin_for_wp__visual',
            array(
        ...
        add_settings_field(
            ...
            function () {
                $this->renderCheckboxInput('recipe_plugin_for_wp__show_box_shadow', $this->showBoxShadowDefault, __('Show box shadow', 'recipe-creator'));
            },
            'recipe_plugin_for_wp__general',
            'recipe_plugin_for_wp__visual',
            array(
    ...
    public function registerBlocks()
    {
        ...
        wp_set_script_translations('recipe-creator-recipe-editor-script', 'recipe-creator', dirname(plugin_dir_path(__FILE__), 1) . '/languages/');
        wp_set_script_translations('recipe-creator-jump-to-recipe-editor-script', 'recipe-creator', dirname(plugin_dir_path(__FILE__), 1) . '/languages/');
    }
    ...
    public function getDummyData()
    {
        return array(
            ...
            "description" => __("You have bananas left over again and don't know what to do with them? How about a delicious shake?", 'recipe-creator'),
            'totalTime' => 5,
            ...
            "utensils" => '<li>' . join("</li><li>", [
                __("Knife", 'recipe-creator'),
                __("Blender", 'recipe-creator'),
            ]) . '</li>',
            "preparationStepsGroups" => array(
                array(
                    ...
                    "list" => '<li>' . join("</li><li>", [
                        ...
                        __("Put all the ingredients in the blender and mix everything for 30 seconds.", 'recipe-creator'),
    ...
    private function getStyleOptions()
    {
        return array(
            ...
            "primaryColorContrast" => get_option('recipe_plugin_for_wp__primary_color_contrast', $this->primaryColorContrastDefault),
            "primaryColorLight" => get_option('recipe_plugin_for_wp__primary_color_light', $this->primaryColorLightDefault),
            "primaryColorLightContrast" => get_option('recipe_plugin_for_wp__primary_color_light_contrast', $this->primaryColorLightContrastDefault),
            "primaryColorDark" => get_option('recipe_plugin_for_wp__primary_color_dark', $this->primaryColorDarkDefault),
            "secondaryColor" => get_option('recipe_plugin_for_wp__secondary_color', $this->secondaryColorDefault),
            "secondaryColorContrast" => get_option('recipe_plugin_for_wp__secondary_color_contrast', $this->secondaryColorContrastDefault),
            "backgroundColor" => get_option('recipe_plugin_for_wp__background_color', $this->backgroundColorDefault),
            "backgroundColorContrast" => get_option('recipe_plugin_for_wp__background_color_contrast', $this->backgroundColorContrastDefault),
