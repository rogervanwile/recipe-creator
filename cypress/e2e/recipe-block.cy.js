function checkRecipeBockScreenshot(name) {
  ...
  cy.get(".foodblogkitchen-toolkit--recipe-block--video-wrapper").invoke(
    "css",
    "display",
    "none"
  );
...
describe("Tests the recipe block", () => {
  ...
  it.only("should be possible to enter a recipe with valid license", () => {
    ...
    cy.get(
      ".foodblogkitchen-toolkit--recipe-block--timings li:nth-child(2) input"
    ).type("15", { force: true });
    cy.get(
      ".foodblogkitchen-toolkit--recipe-block--timings li:nth-child(3) input"
    ).type("20", { force: true });
    cy.get(
      ".foodblogkitchen-toolkit--recipe-block--timings li:nth-child(4) input"
    ).type("25", { force: true });
    cy.contains("70 Min.");
    cy.get(
      ".foodblogkitchen-toolkit--recipe-block--flex-container input:nth-child(1)"
    )
      .clear()
      .type("4", { force: true });
    cy.get(
      ".foodblogkitchen-toolkit--recipe-block--ingredients .foodblogkitchen-toolkit--recipe-block--editor ul"
    ).type("3 bananans\n2 onions", { force: true });
    cy.get(
    ...
    cy.get(".foodblogkitchen-toolkit--recipe-block").contains("2,5 onions");
